    ...
    firstPagePromise.then(pdfPage => {
      ...
      Promise.all([_ui_utils_js__WEBPACK_IMPORTED_MODULE_0__.animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async ([timeStamp, stored, pageLayout, pageMode, openAction]) => {
        ...
        if (stored?.page && viewOnLoad !== ViewOnLoad.INITIAL) {
          ...
          if (pageNum) {
            hash = `page=${pageNum}&zoom=${zoom || stored.zoom},` + `${stored.scrollLeft},${750}`;
          }
...
function scroll(delta, direction) {
  ...
  if (direction == 'y') {
    ...
    $("#viewer").css({
      'transform': 'translateX(' + -app.xTranslate + 'px)'
    });
  }
}
function track() {
...
const pdfjsVersion = '4.1.24';
const pdfjsBuild = '3a48950d8';
const AppConstants = {
